...
class BaseClass{
    public static function getMusic($track_id){
        ...
        $music_info=array('title'=>$arr['results'][0]['trackName'],'artist'=>$arr['results'][0]['artistName'],'artwork_url'=>$arr['results'][0]['artworkUrl100'],'music_url'=>$arr['results'][0]['previewUrl'],'itunes_url'=>$arr['results'][0]['trackViewUrl'],'track_id'=>$arr['results'][0]['trackId']);
        return $music_info;
    }
    ...
    public static function SearchMusic($word){
        ...
        $url="http://ax.itunes.apple.com/WebObjects/MZStoreServices.woa/wa/wsSearch?term=".$word."&media=music&country=jp&lang=ja_jp&limit=20";
        $json = file_get_contents($url);
        $json = mb_convert_encoding($json, 'UTF8', 'ASCII,JIS,UTF-8,EUC-JP,SJIS-WIN');
        $arr = json_decode($json,true);
        if($arr['results']!=null){
