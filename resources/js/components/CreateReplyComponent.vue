<template>
    <section id="create_post">
        <form id="create_post_form" action="/home/detail/send_reply_process" method="post" enctype='multipart/form-data'><!--投稿フォーム-->
            <input type="hidden" name="_token" :value="csrf"><!--csrfトークン-->
            <input type="hidden" name="reply_post_id" :value="replyId"><!--csrfトークン-->
            <textarea name="contents" id="" cols="30" rows="6" :placeholder="placeholderText"></textarea>
        ...
        <div id="post_medias">
            ...
            <div id="post_search_music_parts">
                <search-music-component v-if="showed" ref="search_music" @selected-music="setMusic" @stop-audios="stopAudios"></search-music-component>
            </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            stopSearchAudio(){
                if(this.$refs.search_music!=null){
                    this.$refs.search_music.stopAudio();
                }
            },
            stopAudios(){//全ての再生を止める
                this.$emit('stop-all-music');//全ての曲をとめる
            },
            showSearchBar(){
                if(this.showed){
                    this.showed=false;
